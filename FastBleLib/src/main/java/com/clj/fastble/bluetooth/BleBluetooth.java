...
public class BleBluetooth {
    ...
    public synchronized BluetoothGatt connect(BleDevice bleDevice,
                                              boolean autoConnect,
                                              BleGattCallback callback) {
        return connect(bleDevice, autoConnect, callback, 1);
    }
    ...
    public synchronized BluetoothGatt connect(BleDevice bleDevice,
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            bluetoothGatt = bleDevice.getDevice().connectGatt(BleManager.getInstance().getContext(),
                    true, coreGattCallback, TRANSPORT_LE);
            Log.d("mGattCallback300", "--" + 300);
        } else {
            bluetoothGatt = bleDevice.getDevice().connectGatt(BleManager.getInstance().getContext(),
                    true, coreGattCallback);
            Log.d("mGattCallback301", "--" + 301);
        }
        if (bluetoothGatt != null) {
            if (bleGattCallback != null) {
                bleGattCallback.onStartConnect();
                Log.d("mGattCallback302", "--" + 302);
            }
...
//                    connectGatt();
    ...
    public BluetoothGattCallback coreGattCallback = new BluetoothGattCallback() {
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            ...
            Log.d("jjdjjjjj", "---" + status + newState + gatt);
        ...
        public void onCharacteristicRead(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
            ...
            Log.d("ikkkkkkk", "---suxcee" + status);

            if (gatt.GATT_SUCCESS == status) {
                Log.d("ikkkkkkk", "---suxcee");
                // characteristic was read successful
            } else if (gatt.GATT_INSUFFICIENT_AUTHENTICATION == status ||
                    gatt.GATT_INSUFFICIENT_ENCRYPTION == status) {
                /*
                ...
                Log.d("ikkkkkkk", "---failed");
            } else {
                Log.d("ikkkkkkk", "---elsss");
